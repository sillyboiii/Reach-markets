...
import { ArrowLeft, TrendingUp, TrendingDown, DollarSign, Clock, Activity, Share2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setMarket((prev: any) => {
        ...
        return {
          ...prev,
          currentOdds: calculateOdds(creator.currentScore, prev.threshold),
        };
      });
    }, 30000);
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '32px', display: 'grid', gridTemplateColumns: '1fr 400px', gap: '24px' }}>
        ...
        <div>
          ...
          <div style={{ borderBottom: '1px solid rgba(55, 65, 81, 0.3)', marginBottom: '24px' }}>
            <button onClick={() => setTab('trade')} className={`tab-button ${tab === 'trade' ? 'active' : ''}`}>
              Trade
            </button>
            <button onClick={() => setTab('info')} className={`tab-button ${tab === 'info' ? 'active' : ''}`}>
              Info
            </button>
          </div>
          ...
          {tab === 'trade' ? (
            <div className="glass-card" style={{ padding: '24px' }}>
              <h3 style={{ margin: '0 0 20px 0', fontSize: '18px', fontWeight: 600, color: '#f9fafb' }}>
                24-Hour Reach Score
              </h3>
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={creator.history.slice(-12)}>
                  <defs>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginTop: '24px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 700, color: '#f9fafb' }}>
                    {creator.history[creator.history.length - 1]?.googleTrends || 0}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '4px' }}>Mentions</div>
                  <div style={{ fontSize: '20px', fontWeight: 700, color: '#f9fafb' }}>
                    {creator.history[creator.history.length - 1]?.twitterMentions || 0}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '4px' }}>Reddit Posts</div>
                  <div style={{ fontSize: '20px', fontWeight: 700, color: '#f9fafb' }}>
                    {creator.history[creator.history.length - 1]?.redditPosts || 0}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '4px' }}>YT Views</div>
                  <div style={{ fontSize: '20px', fontWeight: 700, color: '#f9fafb' }}>
                    {((creator.history[creator.history.length - 1]?.youtubeViews || 0) / 1000).toFixed(0)}k
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(55, 65, 81, 0.3)' }}>
                  <span style={{ color: '#9ca3af' }}>Total volume</span>
                  <span style={{ color: '#f9fafb', fontWeight: 500 }}>{formatCurrency(market.totalStaked)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(55, 65, 81, 0.3)' }}>
                  <span style={{ color: '#9ca3af' }}>Market closes</span>
                  <span style={{ color: '#f9fafb', fontWeight: 500 }}>{formatTime(market.endTime)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(55, 65, 81, 0.3)' }}>
                  <span style={{ color: '#9ca3af' }}>Confidence score</span>
                  <span style={{ color: '#f9fafb', fontWeight: 500 }}>{(creator.confidence * 100).toFixed(0)}%</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0' }}>
                  <span style={{ color: '#9ca3af' }}>Threshold</span>
                  <span style={{ color: '#f9fafb', fontWeight: 500 }}>{market.threshold}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div>
          ...
          {/* Positions */}
          <div className="glass-card" style={{ padding: '24px' }}>
            <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: 600, color: '#f9fafb' }}>
              Your Positions ({userBets.length})
            </h3>
            {userBets.length === 0 ? (
                  ...
                  <div key={bet.id} style={{ background: 'rgba(31, 41, 55, 0.4)', border: '1px solid rgba(55, 65, 81, 0.3)', borderRadius: '8px', padding: '16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontSize: '14px', fontWeight: 500, color: '#f9fafb' }}>
                        {bet.type === 'over' ? 'YES' : 'NO'}
                      </span>
                      <span style={{ fontSize: '14px', fontWeight: 500, color: bet.type === 'over' ? '#34d399' : '#f87171' }}>
